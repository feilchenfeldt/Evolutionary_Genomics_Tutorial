...
all(metadata$Sample_ID == sample_ids)
...
[^1]: You can take a look at their *extensive* [documentation](https://jokergoo.github.io/ComplexHeatmap-reference/). 
...
Now, we will use the genomic pairwise distance matrix to create a phylogenetic tree of our samples. We can build a neighbour-joining tree using the `ape` package and visualize it using the `plot` function.[^2]

[^2]: The `plot`function is a generic function that can be overloaded by authors of different packages. 
